import { FileText, Download, Calendar, Search, Plus, Eye, Shield, AlertTriangle, X } from 'lucide-react';
...
export function MDAReportsView() {
  ...
  const handleDownload = (report: MDAVerificationReport) => {
    ...
    openVerificationReport({
      ...
      certificates: [],
      lastVerified: new Date(report.generated_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }),
  ...
  const submitGenerate = async () => {
    ...
    try {
      ...
      showToast('success', 'Report Generated', `Verification report ready for ${created.rc_number}`);
    ...
    <div className="flex-1 h-full overflow-y-auto bg-background">
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        {reports.isLoading ? (
          ...
          <EmptyState icon={FileText} title="Unable to Load Reports" description="Please try again." />
                ...
                {filteredReports.map((report) => {
                  ...
                  return (
                    <tr key={report.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <code className="text-sm bg-muted px-2 py-1 rounded">{report.rc_number}</code>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${risk.cls}`}>
          ...
          <div className="bg-card rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-muted-foreground mt-1 text-sm">Creates a PDF with a QR code to a live verification URL.</p>
              </div>
              <button onClick={() => setIsGenerateModalOpen(false)} className="p-2 hover:bg-muted rounded-md transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-card rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-start justify-between mb-6">
              ...
              <button onClick={() => setViewReportId(null)} className="p-2 hover:bg-muted rounded-md transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>

...
const REFERRALS: Referral[] = [
  {
    ...
    employeeCount: 45,
    sector: 'ICT',
    contactEmail: 'amaka@techventures.ng',
    contactPhone: '+234 801 234 5678',
    referredDate: 'May 8, 2026',
    status: 'new',
    estimatedPremium: 540000,
  },
  {
    id: 'r2',
    ...
    employeeCount: 120,
    sector: 'Construction',
    contactEmail: 'hr@buildright.ng',
    contactPhone: '+234 802 345 6789',
    referredDate: 'May 6, 2026',
    status: 'contacted',
    estimatedPremium: 1440000,
  },
  {
    id: 'r3',
    companyName: 'Lagos Pharma Ltd',
    rcNumber: 'RC5678901',
    employeeCount: 30,
    sector: 'Health',
    contactEmail: 'ceo@lagospharma.ng',
    contactPhone: '+234 803 456 7890',
    referredDate: 'May 4, 2026',
    status: 'enrolled',
    estimatedPremium: 360000,
  },
  {
    id: 'r4',
    companyName: 'Delta Power Solutions',
    rcNumber: 'RC6789012',
    employeeCount: 55,
    sector: 'Energy',
    contactEmail: 'admin@deltapower.ng',
    contactPhone: '+234 804 567 8901',
    referredDate: 'May 2, 2026',
    status: 'contacted',
    estimatedPremium: 660000,
  },
  {
    id: 'r5',
    companyName: 'FoodChain Nigeria',
    rcNumber: 'RC7890123',
    employeeCount: 22,
    sector: 'Agriculture',
    contactEmail: 'hr@foodchain.ng',
    contactPhone: '+234 805 678 9012',
    referredDate: 'Apr 28, 2026',
    status: 'new',
    estimatedPremium: 264000,
  },
  {
    id: 'r6',
    ...
    employeeCount: 80,
    sector: 'Transport',
    contactEmail: 'ops@apexlogistics.ng',
    contactPhone: '+234 806 789 0123',
    referredDate: 'Apr 25, 2026',
    status: 'declined',
    estimatedPremium: 960000,
  },
  {
    id: 'r7',
    companyName: 'Sunrise Media Group',
    rcNumber: 'RC9012345',
    employeeCount: 18,
    sector: 'Media',
    contactEmail: 'admin@sunrisemedia.ng',
    contactPhone: '+234 807 890 1234',
    referredDate: 'Apr 22, 2026',
    status: 'new',
    estimatedPremium: 216000,
  },
];
...
const ENROLLMENTS: Enrollment[] = [
  {
    ...
    rcNumber: 'RC5678901',
    employeeCount: 30,
    enrollmentDate: 'May 5, 2026',
    status: 'certificate-issued',
    nhiaCertDate: 'May 9, 2026',
    monthlyPremium: 360000,
  },
  {
    id: 'e2',
    companyName: 'Kano Textiles Co',
    rcNumber: 'RC2345678',
    employeeCount: 65,
    enrollmentDate: 'Apr 28, 2026',
    status: 'active',
    nhiaCertDate: 'May 3, 2026',
    monthlyPremium: 780000,
  },
  {
    id: 'e3',
    companyName: 'BuildRight Construction',
    rcNumber: 'RC4567890',
    employeeCount: 120,
    enrollmentDate: 'May 7, 2026',
    status: 'under-review',
    monthlyPremium: 1440000,
  },
  {
    id: 'e4',
    companyName: 'Zenith Agro',
    rcNumber: 'RC3456789',
    employeeCount: 40,
    enrollmentDate: 'May 1, 2026',
    status: 'details-received',
    monthlyPremium: 480000,
  },
];
...
function HMOOverview() {
  ...
  const totalEnrolled = ENROLLMENTS.filter(
    (e) => e.status === 'active' || e.status === 'certificate-issued'
  ).length;
  const pendingCommissions = COMMISSIONS.filter((c) => c.status === 'processing').reduce(
    (s, c) => s + c.amount,
    0
  );
  const totalEarned = COMMISSIONS.filter((c) => c.status === 'paid').reduce(
    (s, c) => s + c.amount,
    0
  );
      ...
      <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
        <header className="mb-6">
          ...
          <p className="text-muted-foreground" style={{ fontSize: '15px' }}>
            Axia Health Insurance · Fatima Okafor
          </p>
        </header>
                ...
                return (
                  <div key={r.id} className="flex items-center justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <p style={{ fontSize: '14px', fontWeight: 500 }} className="truncate">
                        {r.companyName}
                      </p>
                      <p className="text-muted-foreground" style={{ fontSize: '12px' }}>
                        {r.rcNumber} · {r.employeeCount} employees
                      </p>
                    </div>
                    <span
                ...
                return (
                  <div key={e.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-muted-foreground" style={{ fontSize: '12px' }}>
                        {cfg.label} · {e.employeeCount} employees
                      </p>
                    </div>
...
function HMOReferrals() {
  ...
  const [filter, setFilter] = useState<'all' | 'new' | 'contacted' | 'enrolled' | 'declined'>(
    'all'
  );
  ...
  const handleContact = (r: Referral) => {
    showToast(
      'success',
      'Contact Initiated',
      `Reaching out to ${r.companyName} at ${r.contactEmail}`
    );
    setSelected(null);
  };
    ...
    <div className="flex-1 h-full overflow-y-auto bg-background">
      <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
        <header className="mb-6">
          ...
          <p className="text-muted-foreground" style={{ fontSize: '15px' }}>
            Companies referred from ClearPass that need NHIA enrollment
          </p>
        </header>
          ...
          {(['all', 'new', 'contacted', 'enrolled', 'declined'] as const).map((f) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full border text-sm capitalize transition-colors ${
                filter === f
                  ? 'border-[#1FC16B] text-[#1FC16B] bg-[#1FC16B]/5'
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    >
                      Details
                      <ChevronRight
                        className={`w-3.5 h-3.5 transition-transform ${selected?.id === r.id ? 'rotate-90' : ''}`}
                      />
                    </button>
    ...
    <div className="flex-1 h-full overflow-y-auto bg-background">
      <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
        <header className="mb-6">
          ...
          <p className="text-muted-foreground" style={{ fontSize: '15px' }}>
            Track enrollment progress for each company
          </p>
        </header>
              ...
              <div key={e.id} className="bg-card border border-border rounded-lg p-4 sm:p-5">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                  <div>
                    ...
                    {e.nhiaCertDate && (
                      <p className="text-muted-foreground" style={{ fontSize: '12px' }}>
                        NHIA cert issued: {e.nhiaCertDate}
                      </p>
                    )}
                  </div>
                  <span
                    ...
                    return (
                      <div key={step} className="flex-1 flex flex-col items-center gap-1">
                        ...
                        <p
                          style={{
                            fontSize: '10px',
                            color: done || active ? '#1FC16B' : 'var(--muted-foreground)',
                          }}
                  ...
                  <button
                    onClick={() =>
                      showToast(
                        'success',
                        'Status Updated',
                        `${e.companyName} moved to next enrollment stage`
                      )
                    }
...
function HMOCommissions() {
  const { showToast } = useToast();
  const totalPaid = COMMISSIONS.filter((c) => c.status === 'paid').reduce(
    (s, c) => s + c.amount,
    0
  );
  const totalPending = COMMISSIONS.filter((c) => c.status !== 'paid').reduce(
    (s, c) => s + c.amount,
    0
  );
    ...
    <div className="flex-1 h-full overflow-y-auto bg-background">
      <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
        <header className="mb-6">
          ...
          <p className="text-muted-foreground" style={{ fontSize: '15px' }}>
            8% commission on monthly premiums for referred enrollments
          </p>
        </header>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                {['Company', 'Period', 'Employees', 'Premium', 'Rate', 'Commission', 'Status'].map(
                  (h) => (
                    <th
                      key={h}
              ...
              {COMMISSIONS.map((c) => (
                ...
                >
                  ...
                  <td
                    ...
                    style={{ fontSize: '14px', color: '#1FC16B' }}
                  >
                    {fmt(c.amount)}
                  </td>
            ...
            <div key={c.id} className="bg-card border border-border rounded-lg p-4">
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground" style={{ fontSize: '13px' }}>
                  {c.employees} employees · {fmt(c.monthlyPremium)}/mo
                </span>
                <span style={{ fontSize: '14px', fontWeight: 700, color: '#1FC16B' }}>
                  {fmt(c.amount)}
                </span>
              </div>
            </div>
...
function HMOAnalytics() {
  const conversionRate = Math.round(
    (REFERRALS.filter((r) => r.status === 'enrolled').length / REFERRALS.length) * 100
  );
  const avgPremium = Math.round(
    ENROLLMENTS.reduce((s, e) => s + e.monthlyPremium, 0) / ENROLLMENTS.length
  );
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          {[
            ...
            },
            {
              ...
              value: fmt(
                COMMISSIONS.filter((c) => c.period === 'May 2026').reduce((s, c) => s + c.amount, 0)
              ),
              icon: BarChart2,
              color: '#1FC16B',
            },

...
export function useGamification() {
      ...
      ACHIEVEMENTS.forEach((achievement) => {
        ...
        switch (achievement.type) {
          case AchievementType.FIRST_CERTIFICATE:
            shouldUnlock =
              action === GamificationAction.UPLOAD_CERTIFICATE && state.totalCertificates === 0;
            break;
          case AchievementType.CERTIFICATE_MASTER:
            shouldUnlock =
              action === GamificationAction.UPLOAD_CERTIFICATE && state.totalCertificates + 1 >= 6;
            break;
          case AchievementType.FIRST_VERIFICATION:
            shouldUnlock =
              action === GamificationAction.VERIFY_COMPANY && state.totalVerifications === 0;
            break;
          case AchievementType.VERIFICATION_EXPERT:
            shouldUnlock =
              action === GamificationAction.VERIFY_COMPANY && state.totalVerifications + 1 >= 10;
            break;
          ...
          case AchievementType.FIRST_REPORT:
            shouldUnlock =
              action === GamificationAction.GENERATE_REPORT && state.totalReports === 0;
            break;
          case AchievementType.REPORT_POWER_USER:
            shouldUnlock =
              action === GamificationAction.GENERATE_REPORT && state.totalReports + 1 >= 5;
            break;
          ...
          case AchievementType.COMPLIANCE_CHAMPION:
            shouldUnlock =
              action === GamificationAction.SCORE_UPDATE &&
              state.currentScore >= 90 &&
              state.streakDays >= 14;
            break;
        }
  ...
  const awardXP = useCallback(
    (action: GamificationAction, metadata?: Record<string, any>) => {
      const event: XPEvent = {
        id: `${action}-${Date.now()}`,
        action,
